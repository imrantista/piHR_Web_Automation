...
// --- Define API paths for multiple maps ---
const allApiPaths = {
  default: {
  ...
  leaveDashboard: {
    validSubdomainApi: "/v2/api/is-valid-subdomain",
    userSessionApi: "/api/v2/pihr-web/user-sessions",
    notificationsApi: "/api/v2/my-dashboard/notifications",
    userQuickLinksApi: "/api/v2/users/user-quick-links",
    wingSliApi: "/api/v2/wings/wing-sli",
...
// --- Helper to build a single API map ---
function buildApiMap(paths) {
  const map = {};
  for (const [key, path] of Object.entries(paths)) {
    const prefix =
      key === "logoutApi" || key === "validSubdomainApi"
        ? SubDomain_URL
        : BASE_URL;
...
// --- Build all maps dynamically ---
const allApiMaps = {};
for (const [mapName, paths] of Object.entries(allApiPaths)) {
  allApiMaps[mapName] = buildApiMap(paths);
}

// --- Export all maps ---
export default allApiMaps;
